...
async def run_test():
    ...
    try:
        ...
        # -> Locate and navigate to the sign up page.
        frame = context.pages[-1]
        # Click on 'Sign In' button to find sign up option or navigate to sign up page
        elem = frame.locator('xpath=html/body/div/div[2]/nav/div/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on 'Don't have an account? Sign up' button to go to the sign-up page.
        frame = context.pages[-1]
        ...
        # -> Input valid full name, email, and password, then submit the registration form.
        frame = context.pages[-1]
        # Input full name
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Email verification failed').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The test plan execution has failed. Verification email was not sent or email verification did not complete successfully, preventing access to the dashboard.")
